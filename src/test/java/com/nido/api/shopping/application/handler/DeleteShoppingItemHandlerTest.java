...
class DeleteShoppingItemHandlerTest {

    @Mock ShoppingItemRepository itemRepository;
    private DeleteShoppingItemHandler handler;
    private final UUID spaceId = UUID.randomUUID();
    private final UUID itemId = UUID.randomUUID();

    @BeforeEach
    void setUp() {
        handler = new DeleteShoppingItemHandler(itemRepository);
    }
    ...
    @Test
    void a_member_can_delete_an_item_in_their_space() {
        when(itemRepository.findById(itemId)).thenReturn(Optional.of(item(spaceId)));

        handler.delete(itemId, spaceId, membership(SpaceRole.MEMBER));
    ...
    void an_item_from_another_space_is_not_found() {
        ...
        assertThatThrownBy(() -> handler.delete(itemId, spaceId, membership(SpaceRole.MEMBER)))
            .isInstanceOf(ShoppingException.ItemNotFound.class);
    }

    @Test
    void a_viewer_cannot_delete_an_item() {
        assertThatThrownBy(() -> handler.delete(itemId, spaceId, membership(SpaceRole.VIEWER)))
