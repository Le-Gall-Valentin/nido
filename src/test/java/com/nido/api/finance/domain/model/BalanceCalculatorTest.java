...
class BalanceCalculatorTest {
    ...
    private static Transaction sharedExpense(UUID payer, BigDecimal amount, List<Contribution> contributors) {
        return new Transaction(UUID.randomUUID(), UUID.randomUUID(), "T", amount, TransactionType.EXPENSE, CATEGORY,
            LocalDate.of(2026, 1, 1), payer, contributors, null, Instant.now());
    }

    @Test
    void a_transaction_with_no_contributors_does_not_affect_any_balance() {
        Transaction personalExpense = sharedExpense(ALICE, new BigDecimal("50.00"), List.of());
    ...
    void the_payer_is_owed_by_each_contributor_for_their_share_when_split_two_ways() {
        Transaction dinner = sharedExpense(ALICE, new BigDecimal("40.00"),
    ...
    void three_members_with_a_custom_split_net_out_correctly() {
        Transaction rent = sharedExpense(ALICE, new BigDecimal("100.00"), List.of(
    ...
    void suggested_transfers_settle_every_member_to_zero_with_the_fewest_transfers() {
        ...
        Transaction outing = sharedExpense(ALICE, new BigDecimal("90.00"), List.of(
