...
public class DataSeeder {
    ...
    public DataSeeder(BoilerplateProperties properties, SeedUseCase seedUseCase) {
    ...
    public void seed() {
        BoilerplateProperties.SeedProperties seed = properties.seed();
        if (seed == null || seed.password() == null || seed.password().isBlank()) {
            throw new IllegalStateException(
                "BOILERPLATE_SEED_PASSWORD must be set — the initial SUPER_ADMIN cannot be created without it"
            );
        }
        try {
