...
public interface UserTotpJpaRepository extends JpaRepository<UserTotpEntity, UUID> {
    ...
    @Query("UPDATE UserTotpEntity u SET u.totpEnabled = true, u.totpSecret = :secret WHERE u.userId = :id")
    void enableTotpById(@Param("id") UUID id, @Param("secret") String secret);
