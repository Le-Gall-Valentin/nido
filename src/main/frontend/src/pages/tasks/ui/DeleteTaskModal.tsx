...
export function DeleteTaskModal({ taskTitle, onClose, onDelete }: DeleteTaskModalProps) {
  ...
  return (
    <Dialog open onClose={onClose} title={t('delete_confirm.title', { title: taskTitle })}>
      <div className="mb-[15px] grid size-[46px] place-items-center rounded-[13px] bg-status-red-dim text-status-red">
        <AlertTriangle className="size-6" />
      </div>
      <div className="mb-5">
        <h3 className="text-[19px] font-semibold text-fg-0 mb-2">{t('delete_confirm.title', { title: taskTitle })}</h3>
        <p className="text-sm text-fg-2 leading-relaxed">{t('delete_confirm.message')}</p>
      </div>

      {hasError && <Alert variant="error" className="mb-4">{t('delete_confirm.error')}</Alert>}

      <div className="flex justify-end gap-2">
        <Button type="button" onClick={onClose} disabled={isLoading}>{t('delete_confirm.cancel')}</Button>
        <Button
