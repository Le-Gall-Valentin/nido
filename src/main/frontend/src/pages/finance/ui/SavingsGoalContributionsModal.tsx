...
export function SavingsGoalContributionsModal({ goalName, contributions, memberLabel, onClose }: SavingsGoalContributionsModalProps) {
  const { t } = useTranslation('finance')

  return (
    <Dialog open onClose={onClose} title={goalName} maxWidth="max-w-lg">
      <div className="mb-4 flex items-center gap-3">
        <div className="grid size-9 shrink-0 place-items-center rounded-[10px] bg-accent-dim text-accent">
          <PiggyBank size={18} />
        </div>
